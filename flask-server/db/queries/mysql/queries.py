...
class MysqlQueries(object):
    ...
    def setEvents(self,parmas,conn):
        query = 'INSERT INTO events(user_id, start_date_time,end_date_time, type, discription, title, watch_platform) VALUES(%s,%s,%s,%s,%s,%s,%s)'
        try:
            cur = conn.cursor()
            cur.execute(query, parmas)
            if cur.rowcount:
    ...
    def getWeeklyEvent(self,parmas,conn):
        query = 'select * from events where start_date_time between %s and %s  order by start_date_time'
        try:
            cur = conn.cursor()
            cur.execute(query, parmas)
